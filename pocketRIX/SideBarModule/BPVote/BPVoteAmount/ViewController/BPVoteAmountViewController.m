...
//    self.model.RIX_balance = @"50";
//    self.model.RIX_net_weight = @"10";
//    self.model.RIX_cpu_weight = @"5";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%.4f%@", (self.model.RIX_net_weight.doubleValue + self.model.RIX_cpu_weight.doubleValue+ self.headerView.amountTF.text.doubleValue )* 10000 * self.nowVoteWeight.doubleValue / 1000000000000, NSLocalizedString(@"亿票", nil)];
    return cell;
}
...
- (void)confirmBtnDidClick:(UIButton *)sender{
    ...
    if ([self.myVoteInfoResult.code isEqualToNumber:@2]) {
        ...
        if (self.headerView.amountTF.text.doubleValue > 0) {
            ...
            if ((self.model.RIX_cpu_weight.doubleValue > 0) && (self.model.RIX_net_weight.doubleValue > 0)) {
...
- (void)registerToVoteSystem{
    ...
    self.registerAccountToVoteSystem_Abi_json_to_bin_request.code = ContractName_ARISEN;
...
#pragma mark -- [@"data"]
    self.approve_Abi_json_to_bin_request.stake_net_quantity = [NSString stringWithFormat:@"%.4f RIX",  self.headerView.amountTF.text.doubleValue/2];
    self.approve_Abi_json_to_bin_request.stake_cpu_quantity = [NSString stringWithFormat:@"%.4f RIX",  self.headerView.amountTF.text.doubleValue/2];
...
- (void)pushTransaction{
    ...
    self.voteProducers_Abi_json_to_bin_request.code = ContractName_ARISEN;
