...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([str isEqualToString:NSLocalizedString(@"设为主账号", nil)]) {
        ...
        NSLog(@"保护隐私");
    }else if([str isEqualToString:NSLocalizedString(@"RIX资源管理", nil)]){
    ...
    }else if([str isEqualToString:NSLocalizedString(@"RIX一键赎回", nil)]){
...
- (void)setToMainAccountBtnDidClick:(UISwitch *)sender{
    ...
    self.setMainAccountRequest.RIXAccountName = self.model.account_name;
...
- (void)setProtectPrivate:(UISwitch *)sender{
    WS(weakSelf);
    self.accountPravicyProtectionRequest.RIXAccountName = self.model.account_name;
...
- (void)unStakeBtnClick{
    UnStakeRIXViewController *vc = [[UnStakeRIXViewController alloc] init];
    //    self.currentAccountResult.data.RIX_cpu_weight = @"6";
    //    self.currentAccountResult.data.RIX_net_weight = @"8";
    vc.currentAccountName = self.model.account_name;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)commonDialogHasPasswordTFViewConfirmBtnDidClick:(UIButton *)sender{
    ...
    if (accountArr.count > 1) {
        ...
        //  通知服务器
        self.setMainAccountRequest.uid = CURRENT_WALLET_UID;
        self.setMainAccountRequest.RIXAccountName = model.account_name;
        [self.setMainAccountRequest postDataSuccess:^(id DAO, id data) {
            BaseResult *result = [BaseResult mj_objectWithKeyValues:data];
            if ([result.code isEqualToNumber:@0]) {
                // 1.将所有的账号都设为 非主账号
