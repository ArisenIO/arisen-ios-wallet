...
@property(nonatomic , strong) RIXPrivateKey *ownerPrivateKey;
@property(nonatomic , strong) RIXPrivateKey *activePrivateKey;
...
- (void)createAllKeys{
    self.ownerPrivateKey = [[RIXPrivateKey alloc] initRIXPrivateKey];
    self.activePrivateKey = [[RIXPrivateKey alloc] initRIXPrivateKey];
    NSLog(@"ownerPrivateKey:\n %@ \n ownerPublicKey:\n %@\n activePrivateKey:\n%@\n activePublicKey:\n %@\n", self.ownerPrivateKey.RIXPrivateKey , self.ownerPrivateKey.RIXPublicKey , self.activePrivateKey.RIXPrivateKey, self.activePrivateKey.RIXPublicKey);
}
...
- (void)configCreateAccountOrderRequestParams{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.RIXPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.ownerPrivateKey.RIXPublicKey;
    }else{
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.RIXPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.activePrivateKey.RIXPublicKey;
    }
}
...
- (void)storeAccountInfoToLocalDatabase{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        model.account_owner_public_key = self.ownerPrivateKey.RIXPublicKey;
        model.account_active_public_key = self.ownerPrivateKey.RIXPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.RIXPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.ownerPrivateKey.RIXPrivateKey password:self.password];
    }else{
        model.account_owner_public_key = self.ownerPrivateKey.RIXPublicKey;
        model.account_active_public_key = self.activePrivateKey.RIXPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.RIXPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.activePrivateKey.RIXPrivateKey password:self.password];
    }
