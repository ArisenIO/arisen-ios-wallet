...
#import "RIXPrivateKey.h"
#import "ImportAccountViewController.h"
#import "RIXMappingImportAccountViewController.h"
...
- (void)privacyPolicyBtnDidClick:(UIButton *)sender{
    ...
    vc.rtfFileName = @"PocketRIXPrivacyPolicy";
...
- (void)createAccountBtnDidClick:(UIButton *)sender {
    ...
    if (![ RegularExpression validateRIXAccountName:self.headerView.accountNameTF.text ]) {
...
- (void)createkeys{
    WS(weakSelf);
    RIXPrivateKey *ownerPrivateKey = [[RIXPrivateKey alloc] initRIXPrivateKey];
    RIXPrivateKey *activePrivateKey = [[RIXPrivateKey alloc] initRIXPrivateKey];
    if (LEETHEME_CURRENTTHEME_IS_SOCAIL_MODE) {
        weakSelf.createAccountService.createRIXAccountRequest.uid = CURRENT_WALLET_UID;
    }else if (LEETHEME_CURRENTTHEME_IS_BLACKBOX_MODE){
        weakSelf.createAccountService.createRIXAccountRequest.uid = @"6f1a8e0eb24afb7ddc829f96f9f74e9d";
    }
    weakSelf.createAccountService.createRIXAccountRequest.RIXAccountName = weakSelf.headerView.accountNameTF.text;
    weakSelf.createAccountService.createRIXAccountRequest.ownerKey = ownerPrivateKey.RIXPublicKey;
    weakSelf.createAccountService.createRIXAccountRequest.activeKey = activePrivateKey.RIXPublicKey;
    NSLog(@"{ownerPrivateKey:%@\nRIXPublicKey:%@\nactivePrivateKey:%@\nRIXPublicKey:%@\n}", ownerPrivateKey.RIXPrivateKey, ownerPrivateKey.RIXPublicKey, activePrivateKey.RIXPrivateKey, activePrivateKey.RIXPublicKey);
    // 创建RIX账号
    
    [weakSelf.createAccountService createRIXAccount:^(id service, BOOL isSuccess) {
        ...
        if (isSuccess) {
            ...
            if ([code isEqualToNumber:@0]) {
                ...
                model.account_active_public_key = activePrivateKey.RIXPublicKey;
                model.account_owner_public_key = ownerPrivateKey.RIXPublicKey;
                model.account_active_private_key = [AESCrypt encrypt:activePrivateKey.RIXPrivateKey password:weakSelf.loginPasswordView.inputPasswordTF.text];
                model.account_owner_private_key = [AESCrypt encrypt:ownerPrivateKey.RIXPrivateKey password:weakSelf.loginPasswordView.inputPasswordTF.text];
