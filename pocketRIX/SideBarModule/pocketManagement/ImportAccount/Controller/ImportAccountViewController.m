...
@property(nonatomic , strong) BackupRIXAccountService *backupRIXAccountService;
...
- (BackupRIXAccountService *)backupRIXAccountService{
    if (!_backupRIXAccountService) {
        _backupRIXAccountService = [[BackupRIXAccountService alloc] init];
    }
    return _backupRIXAccountService;
}
...
- (void)validateInputFormat{
    ...
    private_active_Key_is_validate = [RIX_Key_Encode validateWif:self.headerView.private_activeKey_tf.text];
    private_owner_Key_is_validate = [RIX_Key_Encode validateWif:self.headerView.private_ownerKey_TF.text ];
...
- (void)createPublicKeys{
    ...
    active_public_key_from_local = [RIX_Key_Encode RIX_publicKey_with_wif:self.headerView.private_activeKey_tf.text];
    owner_public_key_from_local = [RIX_Key_Encode RIX_publicKey_with_wif:self.headerView.private_ownerKey_TF.text];
    // 请求该账号的公钥
    WS(weakSelf);
...
- (void)configAccountInfo{
    ...
    self.backupRIXAccountService.backupRIXAccountRequest.uid = CURRENT_WALLET_UID;
    self.backupRIXAccountService.backupRIXAccountRequest.RIXAccountName = accountInfo.account_name;
    [self.backupRIXAccountService backupAccount:^(id service, BOOL isSuccess) {
        if (isSuccess) {
            NSLog(@"备份到服务器成功!");
        }
    }];
