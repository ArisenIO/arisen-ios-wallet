    ...
    [self.rixResourceService get_account:^(rixResourceResult *result, BOOL isSuccess) {
        if (isSuccess) {
            ...
            if (cpuArr.count>0 && netArr.count > 0) {
                ...
                weakSelf.headerView.rixAmountLabel.text = [NSString stringWithFormat:@"%.4f", cpuStr.doubleValue + netStr.doubleValue];
            }
        }
    }];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BandwidthManageCellModel *model = self.rixResourceService.dataSourceArray[indexPath.section];
    cell.model = model;
    return cell;
}
...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    return self.rixResourceService.dataSourceArray.count;
}
