...
- (void)buildDataSource{
    WS(weakSelf);
    self.RIXResourceService.getAccountRequest.name = self.accountResult.data.account_name;
    [self.RIXResourceService get_account:^(RIXResourceResult *result, BOOL isSuccess) {
        if (isSuccess) {
            weakSelf.RIXResourceResult = result;
            CGFloat progress = weakSelf.RIXResourceResult.data.ram_usage.doubleValue/weakSelf.RIXResourceResult.data.ram_max.doubleValue;
            weakSelf.headerView.progressView.progress = progress;
            weakSelf.headerView.tipLabel.text = [NSString stringWithFormat:@"%@(%@byte/%@byte)", NSLocalizedString(@"存储配额使用情况", nil), weakSelf.RIXResourceResult.data.ram_usage, weakSelf.RIXResourceResult.data.ram_max];
        }
    }];
}
...
-(void)buyRamBtnDidClick:(UIButton *)sender{
    ...
    vc.pageType = @"buy_ram";
    vc.RIXResourceResult = self.RIXResourceResult;
