...
- (void)changeActionBaseViewLabelTap:(UITapGestureRecognizer *)sender{
    ...
    if (sender.view.tag == 1000) {
        ...
        self.cpuAmountInputTF.placeholder = NSLocalizedString(@"输入RIX数量", nil);
        self.netAmountInputTF.placeholder = NSLocalizedString(@"输入RIX数量", nil);
        self.cpuAmountUnitLabel.text = SymbolName_RIX;
        self.netAmountUnitLabel.text = SymbolName_RIX;
        [self.confirmBtn setTitle:NSLocalizedString(@"确认抵押", nil) forState:(UIControlStateNormal)];
        self.avalibleRIXAmountLabel.hidden = NO;
        ...
        self.avalibleRIXAmountLabel.hidden = YES;
    }
}
...
- (void)updateViewWithRIXResourceResult:(RIXResourceResult *)model{
    self.model = model;
...
//        self.brrowStakeLabel.text = [NSString stringWithFormat:@"%.4f", totalCpuStr.doubleValue + totalNetStr.doubleValue];
    ...
    self.cpuBorrowLabel.text = [NSString stringWithFormat:@"%@ %.4f %@",  NSLocalizedString(@"借用", nil), totalCpuStr.doubleValue - selfCpuStr.doubleValue, SymbolName_RIX];
    ...
    self.netBorrowLabel.text = [NSString stringWithFormat:@"%@ %.4f %@",  NSLocalizedString(@"借用", nil), totalNetStr.doubleValue - selfNetStr.doubleValue, SymbolName_RIX];
    ...
    if (model.data.refund_request) {
        ...
        self.unstakeAmountLabel.text = [NSString stringWithFormat:@"%.4f RIX", model.data.refund_request_cpu_amount.doubleValue  +  model.data.refund_request_net_amount.doubleValue];
    }else{
        self.unstakeTimelabel.text = @"--/--";
        self.unstakeAmountLabel.text = [NSString stringWithFormat:@"0.0000 %@", SymbolName_RIX];
    }
    
    
    self.avalibleRIXAmountLabel.text = [NSString stringWithFormat:@"%@:%@", NSLocalizedString(@"可用", nil), model.data.core_liquid_balance];
