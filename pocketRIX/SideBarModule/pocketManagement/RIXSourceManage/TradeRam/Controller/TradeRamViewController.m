...
- (void)buildDataSource{
    WS(weakSelf);
    self.get_table_rows_request.code = ContractName_ARISEN;
    self.get_table_rows_request.scope = ContractName_ARISEN;
    ...
    [self.get_table_rows_request postOuterDataSuccess:^(id DAO, id data) {
        ...
        if ((quote_balanceStr.length > 4) && (base_balanceStr.length > 4)) {
            ...
            if ([self.pageType isEqualToString: NSLocalizedString(@"buy_ram", nil)]) {
                weakSelf.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f RIX", self.accountResult.data.RIX_balance.doubleValue* weakSelf.headerView.modifyRamSlider.value];
                weakSelf.headerView.predictLabel.text = [NSString stringWithFormat:@"预计配额：%@ bytes", [[self.accountResult.data.RIX_balance yw_stringByDividingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:@"0.5" withRoundingMode:(NSRoundPlain) scale:4] ];
                
                
            }else if ([self.pageType isEqualToString: NSLocalizedString(@"sell_ram", nil)]){
                weakSelf.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f bytes", self.RIXResourceResult.data.ram_max.doubleValue * weakSelf.headerView.modifyRamSlider.value  ];
                weakSelf.headerView.predictLabel.text = [NSString stringWithFormat:@"%@：%@ RIX", NSLocalizedString(@"预计出售价格", nil), [[self.RIXResourceResult.data.ram_max yw_stringByMultiplyingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:@"0.5" withRoundingMode:(NSRoundPlain) scale:4] ];
...
- (void)modifySliderDidSlide:(UISlider *)sender{
    ...
    if ([self.pageType isEqualToString: NSLocalizedString(@"buy_ram", nil)]) {
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f RIX", self.accountResult.data.RIX_balance.doubleValue * progress];
    self.headerView.predictLabel.text =  [NSString stringWithFormat:@"%@ bytes", [[self.accountResult.data.RIX_balance yw_stringByDividingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:[NSString stringWithFormat:@"%.2f", progress] withRoundingMode:(NSRoundPlain) scale:4]];
        
    }else if ([self.pageType isEqualToString: NSLocalizedString(@"sell_ram", nil)]){
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f bytes", self.RIXResourceResult.data.ram_max.doubleValue * progress  ];
        self.headerView.predictLabel.text = [NSString stringWithFormat:@"预计出售价格：%@ RIX", [[self.RIXResourceResult.data.ram_max yw_stringByMultiplyingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:[NSString stringWithFormat:@"%.2f", progress] withRoundingMode:(NSRoundPlain) scale:4]];
    }
}
