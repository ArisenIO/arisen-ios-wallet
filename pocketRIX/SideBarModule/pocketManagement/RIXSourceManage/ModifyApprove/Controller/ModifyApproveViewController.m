...
@property (nonatomic , copy) NSString *RIX_balance_str;
...
- (void)buildDataSource{
    if (self.RIXResourceResult.data.cpu_weight.length > 4 && self.RIXResourceResult.data.net_weight.length > 4) {
       
        if ([self.pageType isEqualToString: NSLocalizedString(@"cpu_bandwidth", nil)]) {
            self.weight_str = [self.RIXResourceResult.data.cpu_weight substringToIndex:self.RIXResourceResult.data.cpu_weight.length-4];
            self.price_str = [[self.RIXResourceResult.data.cpu_weight substringToIndex:self.RIXResourceResult.data.cpu_weight.length-4] yw_stringByDividingBy:self.RIXResourceResult.data.cpu_max withRoundingMode:(NSRoundPlain) scale:8];
            
        }else if ([self.pageType isEqualToString: NSLocalizedString(@"net_bandwidth", nil)]){
            self.weight_str = [self.RIXResourceResult.data.net_weight substringToIndex:self.RIXResourceResult.data.net_weight.length-4];
            self.price_str = [[self.RIXResourceResult.data.net_weight substringToIndex:self.RIXResourceResult.data.net_weight.length-4] yw_stringByDividingBy:self.RIXResourceResult.data.net_max withRoundingMode:(NSRoundPlain) scale:8];
        }
        self.RIX_balance_str = self.accountResult.data.RIX_balance;
        self.canStakeAmount_str = [self.weight_str yw_stringBySubtracting:MIN_AMOUNT withRoundingMode:(NSRoundPlain) scale:8];
        self.total_str = [self.RIX_balance_str yw_stringByAdding:self.canStakeAmount_str withRoundingMode:(NSRoundPlain) scale:8];
...
//            self.headerView.predictLabel.text = [NSString stringWithFormat:@"%@：%@ ms",NSLocalizedString(@"预计配额", nil), [self.weight_str yw_stringByDividingBy:self.price_str  withRoundingMode:(NSRoundPlain) scale:4] ];
        ...
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%@ RIX", self.weight_str];
        self.amountTFAmount_str = self.weight_str;
        
        
        if ((self.RIX_balance_str.doubleValue + self.weight_str.doubleValue) < MIN_AMOUNT.doubleValue) {
...
- (void)modifyApproveSliderDidSlide:(UISlider *)sender{
    ...
    if (progress == 0) {
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%@ RIX", [MIN_AMOUNT yw_stringByAdding:@"0" withRoundingMode:(NSRoundPlain) scale:4]];
...
//            self.headerView.predictLabel.text = [NSString stringWithFormat:@"%@：%@ ms",  NSLocalizedString(@"预计配额", nil),[MIN_AMOUNT yw_stringByDividingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] ];
        ...
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%@ RIX", [MIN_AMOUNT yw_stringByAdding:[self.total_str yw_stringByMultiplyingBy:[NSString stringWithFormat:@"%.2f", progress] withRoundingMode:(NSRoundPlain) scale:8] withRoundingMode:(NSRoundPlain) scale:4]];
...
- (void)approve{
    ...
    if ([self.pageType isEqualToString: NSLocalizedString(@"cpu_bandwidth", nil)]) {
        self.approve_Abi_json_to_bin_request.stake_cpu_quantity = [NSString stringWithFormat:@"%@ RIX", [self.amountTFAmount_str yw_stringBySubtracting:self.weight_str withRoundingMode:(NSRoundPlain) scale:4]];
        self.approve_Abi_json_to_bin_request.stake_net_quantity = @"0.0000 RIX";
    }else if ([self.pageType isEqualToString: NSLocalizedString(@"net_bandwidth", nil)]){
        self.approve_Abi_json_to_bin_request.stake_cpu_quantity = @"0.0000 RIX";
        self.approve_Abi_json_to_bin_request.stake_net_quantity = [NSString stringWithFormat:@"%@ RIX", [self.amountTFAmount_str yw_stringBySubtracting:self.weight_str withRoundingMode:(NSRoundPlain) scale:4]];
    }
    WS(weakSelf);
...
- (void)unStake{
    ...
    if ([self.pageType isEqualToString: NSLocalizedString(@"cpu_bandwidth", nil)]) {
        self.unstakeRIXAbiJsonTobinRequest.unstake_cpu_quantity = [NSString stringWithFormat:@"%@ RIX",  [self.weight_str yw_stringBySubtracting:self.amountTFAmount_str withRoundingMode:(NSRoundPlain) scale:4]];
        self.unstakeRIXAbiJsonTobinRequest.unstake_net_quantity = @"0.0000 RIX";
    }else if ([self.pageType isEqualToString: NSLocalizedString(@"net_bandwidth", nil)]){
        self.unstakeRIXAbiJsonTobinRequest.unstake_cpu_quantity = @"0.0000 RIX";
        self.unstakeRIXAbiJsonTobinRequest.unstake_net_quantity = [NSString stringWithFormat:@"%@ RIX", [self.weight_str yw_stringBySubtracting:self.amountTFAmount_str withRoundingMode:(NSRoundPlain) scale:4]];
    }
    WS(weakSelf);
