...
@property(nonatomic , strong) RIXPrivateKey *privateKey;
@end
...
- (RIXPrivateKey *)privateKey{
    if (!_privateKey) {
        _privateKey = [[RIXPrivateKey alloc] initRIXPrivateKey];
    }
    return _privateKey;
}
...
- (void)validateInputFormat{
    // 验证私钥格式是否正确
    private_Key_is_validate = [RIX_Key_Encode validateWif:self.headerView.textView.text ];
    // 将用户导入的私钥生成公钥
    public_key_from_local = [RIX_Key_Encode RIX_publicKey_with_wif:self.headerView.textView.text];
...
- (void)pushActicon:(NSDictionary *)argsDict{
    self.abi_json_to_binRequest.code = ContractName_ARISEN;
    self.mainService.code = ContractName_ARISEN;
...
- (void)addGeneratePrivateKeyView{
    ...
    self.generatePrivateKeyView.contentTextView.text = self.privateKey.RIXPrivateKey;
