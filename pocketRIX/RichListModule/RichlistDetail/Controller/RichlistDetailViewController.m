...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [[NavigationView alloc] initNavigationViewWithFrame:(CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT)) LeftBtnImgName:@"back_white" titleImage:@"PocketRIX" rightImgName:@"addFollow" delegate:self];
    }
    return _navView;
}
                ...
                if (result.data.count == 0) {
                    ...
                    for (WalletAccount *model in weakSelf.mainService.accountsArray) {
                        if ([model.isMainAccount isEqualToNumber:@1]) {
                            // 获取主账号资产详情
                            if (IsNilOrNull(model.RIXAccountName)) {
                                return;
                            }
                           
                            self.get_token_info_service.get_token_info_request.accountName = model.RIXAccountName;
...
//                                                weakSelf.headerView.model = result.data;
                            ...
                            weakSelf.headerView.userAccountLabel.text = model.RIXAccountName;
//                            weakSelf.mainService.getAccountAssetRequest.name = model.RIXAccountName;
