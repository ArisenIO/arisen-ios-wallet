...
- (void)viewDidLoad {
    ...
    self.headerView.RIXAccountNameLabel.text = CURRENT_ACCOUNT_NAME;
}
...
//    let data = timestamp + account + uuID + ref     //ref为钱包名，标示来源
    ...
    const int8_t *private_key = [[RIX_Key_Encode getRandomBytesDataWithWif:[AESCrypt decrypt:accountInfo.account_active_private_key password:self.loginPasswordView.inputPasswordTF.text]] bytes];
...
- (void)sendSignedResultToDappServer{
    ...
    [params setObject:VALIDATE_STRING(@"PocketRIX") forKey:@"ref"];
