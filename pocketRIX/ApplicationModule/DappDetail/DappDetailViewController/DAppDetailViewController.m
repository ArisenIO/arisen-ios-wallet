...
#define JS_INTERACTION_METHOD_getRixAccount @"getRixAccount"
#define JS_INTERACTION_METHOD_getWalletWithAccount @"getWalletWithAccount"
#define JS_INTERACTION_METHOD_getRixBalance @"getRixBalance"
#define JS_INTERACTION_METHOD_getRixAccountInfo @"getRixAccountInfo"
...
- (WKWebView *)webView{
    if (!_webView) {
        ...
        if (@available(iOS 9.0, *)) {
            self.webView.customUserAgent = @"PocketARISENs";
        } else {
            // Fallback on earlier versions
        }
        self.allowZoom = YES;
...
-(void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    self.allowZoom = NO;
    [self passRixAccountNameToJS];
    [self passWalletInfoToJS];
    WS(weakSelf);
    // 确保js 能收到 RixAccountName
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf passRixAccountNameToJS];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSHACTION] || [self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSH]) {
        ...
        if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getRixAccount]) {
            [self js_pushMessage_method_getRixAccount];
        ...
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getRixBalance]){
            [self js_pushMessage_method_getRixBalance];
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getRixAccountInfo]){
            [self js_pushMessage_method_getRixAccountInfo];
...
- (void)passRixAccountNameToJS{
    NSString *js = [NSString stringWithFormat:@"getRixAccount('%@')", self.choosedAccountName];
...
- (void)js_pushMessage_method_getRixAccount{
    WS(weakSelf);
    [self.dappDetailService getRixAccount:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)js_pushMessage_method_getRixBalance{
    WS(weakSelf);
    GetRixBalanceModel *model = [GetRixBalanceModel mj_objectWithKeyValues:self.dappPushMessageModel.params];
    ...
    [self.dappDetailService getRixBalance:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}

- (void)js_pushMessage_method_getRixAccountInfo{
    WS(weakSelf);
    [self.dappDetailService getRixAccountInfo:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)dappChangeAccountOnNavigationRightViewDidClick{
    ...
    builder.cancelText = NSLocalizedString(@"选择您的RIX账号", nil);
    WS(weakSelf);
...
- (void)loadWebView{
    ...
    if ([self.model.dappUrl isEqualToString:@"https://rixflare.io"]) {
