...
+ (NSString *)rix_publicKey_with_wif:(NSString *)wif{
    ...
    NSString *rixPubKey = [NSString stringWithFormat:@"RIX%@", [NSString stringWithUTF8String:base]];
    return rixPubKey;
}
...
+ (NSString *)encode_rix_PublicKey_with_uecc_publicKey_bytes_data:(NSData *)uecc_publicKey_bytes_data{
    ...
    return [NSString stringWithFormat:@"RIX%@", [NSString stringWithUTF8String:base]];;
}
...
+ (NSData *)decode_rix_publicKey:(NSString *)rix_publicKey{
    if (!(rix_publicKey.length > 0)) {
        NSLog(@"parameter rix_publicKey can't be nil!");
        return nil;
    }
    if (![rix_publicKey hasPrefix:@"RIX"]) {
        NSLog(@"parameter rix_publicKey has not prefix 'RIX'!");
        return nil;
    }
    const char *b58 = [[rix_publicKey substringFromIndex:3] UTF8String];
    unsigned char bin[100];
    size_t binlen = 37;
    unsigned char *hash;
    ...
    for(int i=0;i<4;i++){
        if(validateHash[i]!=hash[i]){
            NSLog(@"parameter rix_publicKey validate failed!");
            return nil;
        }
    }
