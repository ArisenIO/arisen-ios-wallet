...
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andCapacity:(int)capacity{
    RixByteWriter *writer = [[RixByteWriter alloc] initWithCapacity:capacity];
    if (chainId) {
    ...
    //transaction_extensions :: add transaction_extensions in transaction, although it dRix not support yet.
    [writer putVariableUInt:0];
