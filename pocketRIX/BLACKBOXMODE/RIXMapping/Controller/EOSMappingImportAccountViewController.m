//
//  RIXMappingImportAccountViewController.m
//  pocketRIX
//
...
@interface RIXMappingImportAccountViewController ()<UIGestureRecognizerDelegate, NavigationViewDelegate, RIXMappingImportAccountHeaderViewDelegate, LoginPasswordViewDelegate>
@property(nonatomic, strong) NavigationView *navView;
@property(nonatomic, strong) RIXMappingImportAccountHeaderView *headerView;

@property(nonatomic, strong) GetAccountNameByWifRequest *getAccountNameByWifRequest;
@property(nonatomic, strong) LoginPasswordView *loginPasswordView;
@end

@implementation RIXMappingImportAccountViewController
...
- (RIXMappingImportAccountHeaderView *)headerView{
    if (!_headerView) {
        _headerView = [[[NSBundle mainBundle] loadNibNamed:@"RIXMappingImportAccountHeaderView" owner:nil options:nil] firstObject];
...
- (void)importRIXMappingAccountBtnDidClick{
    if (IsStrEmpty(self.headerView.privateKeyTF.text) ) {
        ...
        private_active_Key_is_validate = [RIX_Key_Encode validateWif:self.headerView.privateKeyTF.text];
...
- (void)checkLocalDatabaseAlreadyHasAccountWithAccountName:(NSString *)accountName{
    ...
    [self importRIXMappingAccount];
}

- (void)importRIXMappingAccount{
    // 请求该账号的公钥
    WS(weakSelf);
    active_public_key_from_local = [RIX_Key_Encode RIX_publicKey_with_wif:self.headerView.privateKeyTF.text];
    ...
    [self.getAccountNameByWifRequest postOuterDataSuccess:^(id DAO, id data) {
        ...
        RIXMappingResult *result = [RIXMappingResult mj_objectWithKeyValues:data];
