...
#import "RIXByteWriter.h"
#import "RIX_Key_Encode.h"
#import "Sha256.h"
#import "uECC.h"
...
- (void)pushTransactionRequestOperation{
    ...
    NSLog(@"xpetwif:%@",wif );
    const int8_t *private_key = [[RIX_Key_Encode getRandomBytesDataWithWif:wif] bytes];
    ...
    Sha256 *sha256 = [[Sha256 alloc] initWithData:[RIXByteWriter getBytesForSignatureExcuteMultipleActions:self.chain_Id andParams: [[self.getRequiredPublicKeyRequest parameters] objectForKey:@"transaction"] andCapacity:255]];
    ...
    if (recId == -1 ) {
        ...
        NSString *packed_trxHexStr = [[RIXByteWriter getBytesForSignatureExcuteMultipleActions:nil andParams: [[self.getRequiredPublicKeyRequest parameters] objectForKey:@"transaction"] andCapacity:512] hexadecimalString];
...
//    }
    ...
    Sha256 *sha256 = [[Sha256 alloc] initWithData:[RIXByteWriter getBytesForSignatureExcuteMultipleActions:[NSObject convertHexStrToData:@"aca376f206b8fc25a6ed44dbdc66547c36c6c33e3a119ffbeaef943642f0e906"] andParams: transacDic andCapacity:255]]; // chainId 写死
//    Sha256 *sha256 = [[Sha256 alloc] initWithData:[RIXByteWriter getBytesForSignatureExcuteMultipleActions:[NSObject convertHexStrToData:scatterResult.chainId] andParams: transacDic andCapacity:255]];
    int8_t signature[uECC_BYTES*2];
    int recId = uECC_sign_forbc(private_key, sha256.mHashBytesData.bytes, signature);
    if (recId == -1 ) {
        printf("could not find recid. Was this data signed with this key?\n");
...
- (NSString *)excuteSignatureMessageForScatterWithActor:(NSString *)actor signatureMessage:(NSString *)messageStr andPassword:(NSString *)password{
    ...
    const int8_t *private_key = [[RIX_Key_Encode getRandomBytesDataWithWif:[AESCrypt decrypt:accountInfo.account_active_private_key password:password]] bytes];
//    const int8_t *private_key = [[RIX_Key_Encode getRandomBytesDataWithWif:@"5JLHReCKAn88SdEDtDt8DzMweDdD7eiaoc6w72jWFuVR4piNh5y"] bytes];
    //     [NSObject out_Int8_t:private_key andLength:32];
    if (!private_key) {
        [TOASTVIEW showWithText:@"private_key can't be nil!"];
        return nil;
