...
- (void)configHeaderView{
    ...
    [self.getSparklinesRequest getDataSusscess:^(id DAO, id data) {
        ...
        if ([weakSelf.model.token_symbol isEqualToString:@"RIX"] && [weakSelf.model.contract_name isEqualToString:ContractName_ARISENTOKEN]) {
            tendencyUrlStr = data[@"data"][@"sparkline_rix_png"];
...
- (void)SocialSharePanelViewDidTap:(UITapGestureRecognizer *)sender{
    ...
    if ([self.model.token_symbol isEqualToString:@"RIX"]) {
        model.imageName = @"rix_avatar";
    ...
    model.webPageUrl = @"https://pocketrix.com";
    NSLog(@"%@", platformName);
...
- (void)assestsDetailFooterViewDidClick:(UIButton *)sender{
    if (sender.tag == 1000) {
        ...
        if ([self.model.token_symbol isEqualToString:@"OCT"]  || [self.model.token_symbol isEqualToString:@"RIX"]) {
