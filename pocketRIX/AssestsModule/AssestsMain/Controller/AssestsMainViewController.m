...
#import "RIXResourceManageViewController.h"
...
- (void)loadAllBlocks{
    ...
    [self.navView setChangedBtn1DidClickBlock:^{
            ...
            vc.currentAssestsType = SymbolName_RIX;
            [weakSelf.navigationController pushViewController:vc animated:YES];
    ...
    [self.headerView setRedPacketBtnDidClickBlock:^{
            ...
            for (TokenInfo *token in weakSelf.get_token_info_service.dataSourceArray) {
                if ([token.token_symbol isEqualToString:@"OCT"] || [token.token_symbol isEqualToString:@"RIX"]) {
                    result = YES;
                }
            }
            if (result) {
                ...
                [TOASTVIEW showWithText: NSLocalizedString(@"请关注RIX或者OCT即可发送红包", nil)];
            }
    ...
    [self.headerView setRamTradeBtnDidClickBlock:^{
            ...
            RIXResourceManageViewController *vc = [[RIXResourceManageViewController alloc] init];
...
////        model.url = @"https://coincreate.github.io/RIX_coincreate/coincreate_scatter.html";
//        model.applyName = @"RIX内存市场";
//        vc.model = model;
...
- (void)updateBtnDidClick:(UIButton *)sender{
    [[UIApplication sharedApplication] openURL: [NSURL URLWithString:@"http://pocketRIX.com"]];
}
...
- (void)getAccount{
    WS(weakSelf);
    self.RIXResourceService.getAccountRequest.name = CURRENT_ACCOUNT_NAME;
    [self.RIXResourceService get_account:^(RIXResourceResult *result, BOOL isSuccess) {
        if (isSuccess) {
            [weakSelf.headerView updateViewWithRIXResourceResult:result];
        }
...
- (void)addCommonDialogHasTitleView{
    ...
    model.detail = NSLocalizedString(@"添加RIX账号继续操作", nil);
